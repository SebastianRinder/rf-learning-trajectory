function  sNext = simAcroBot(state, action, ~)
    theta1        = state(1); 
    theta2        = state(2);
    theta1_dot    = state(3);
    theta2_dot    = state(4);
    ...
    accel2 = (action+phi1*(d2/d1)-m2*l1*lc2*theta1_dot*theta1_dot*sin(theta2)-phi2); 
    accel2 = accel2/(m2*lc2Square+I2-(d2*d2/d1)); 
    accel1 = -(d2*accel2+phi1)/d1; 

    for i=1:4
        theta1_dot = theta1_dot + accel1*delta_t;
        theta1_dot = applyBound(theta1_dot, [-maxSpeed1 maxSpeed1]);
        
        theta1 = theta1 + theta1_dot*delta_t; 	    
        theta2_dot = theta2_dot + accel2*delta_t;

        theta2_dot = applyBound(theta2_dot, [-maxSpeed2 maxSpeed2]);
        theta2 = theta2 + theta2_dot*delta_t; 
    end
    
    theta1 = applyBound(theta1, [-pi pi]);
    theta2 = applyBound(theta2, [-pi pi]);
