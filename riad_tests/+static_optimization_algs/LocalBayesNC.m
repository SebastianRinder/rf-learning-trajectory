classdef LocalBayesNC
    
    % assumes the noise to be known
    
    methods (Static)        
        %%
        function sign = getSignature(optimizerInput)
            sign = ['LocalBayesNC' '_' num2str(optimizerInput.epsiKL) '_' num2str(optimizerInput.nbIter) '_' ...
        ...
        function [perf, kls] = optimizeStruct(optimizerInput, func)
            ...
            [perf, kls] = static_optimization_algs.LocalBayesNC.optimize(optimizerInput.initDistrib, optimizerInput.epsiKL, optimizerInput.entropyReduction, ...
        ...
        function [perf, allEvalsReturn, kls] = optimize(initDistrib, epsiKL, entropyReduc, nbSamplesPerIter, nbIter, maxIterReuse, nbPStarSamples, minProbaReuse, fun, gpStuffPath, kernelType, featureFunction, yCenteringType, gpHyperOption, samplingOption, videoFile)
            ...
            elseif(strcmp(kernelType, 'linear'))
                ...
                error('unrecognized kernel in LocalBayesNC');
            end
            ...
            for iter = 1:nbIter
                %%% evaluation
%                 currPerf = static_optimization_algs.LocalBayesNC.distEval(currentDistrib, @(x)fun.eval(x), nbSamplesForEval);
%                 perf(iter, :) = [((iter-1) * nbSamplesPerIter), currPerf];
                ...
                if(isempty(samplesArgmax))
                    ...
                    %newSamples = static_optimization_algs.LocalBayesNC.getLocalArgmaxSamples(nbSamplesPerIter, currentDistrib, gp_rec, gp, x, y, cholPrec, nbSamplesForAcquisition);
                    %newSamples = currentDistrib.getSamples(nbSamplesPerIter);
                    ...
                    elseif(strcmp(samplingOption, 'Random'))
                        ...
                        error('wrong sampling option in LocalBayesNC');
                    end
                end                
...
%                  x = bsxfun(@minus, localSamples, currentDistrib.mu) * cholPrec; %normalize data according to current search distribution
                ...
                elseif(strcmp(yCenteringType, 'max'))
                    ...
                    error('Unrecognized y centering type in LocalBayesNC');
                end
                if(~isempty(featureFunction))
                    x = featureFunction(x);
                end
                [targetDistrib, samplesArgmax, gp, gp_rec] = static_optimization_algs.LocalBayesNC.gaussianArgmax...
                    (currentDistrib, x, y, muz, rotz, gp, gp_rec, gpHyperOption, nbPStarSamples, nbSamplesForAcquisition, featureFunction);
...
%                 targetDistrib = static_optimization_algs.Normal(targetMu, targetCov);
                ...
                % 2- Quad model
                quadModel = static_optimization_algs.LocalBayesNC.gaussToQuad(targetDistrib);
                
                %%% current distrib plotting
                if(~isempty(videoFile))
                    frame = static_optimization_algs.LocalBayesNC.getFrame(fun, currentDistrib, localSamples, x, y, gp_rec, muz, rotz, quadModel, samplesArgmax, iter, featureFunction);
        ...
        function [targetDistrib, samplesArgmax, gp, gp_rec] = gaussianArgmax(currentDistrib, x, y, mu, cholPrec, gp, gp_previousRec, gpHyperOption, nbPStarSamples, nbSamplesForAcquisition, featureFunction)
            ...
            elseif(strcmp(gpHyperOption, 'MAP'))
                ...
                error('wrong hyperparm option in LocalBayesNC');
            end
            
            % fmax sampling dist
            fmaxGauss = static_optimization_algs.Normal(currentDistrib.mu, currentDistrib.getCov);
            %                 fmaxGauss = currentDistrib;
            samplesArgmax = static_optimization_algs.LocalBayesNC.getLocalArgmaxSamples(nbPStarSamples, fmaxGauss, gp_rec, gp, x, y, mu, cholPrec, nbSamplesForAcquisition, featureFunction);
