...
rootPlot = '+static_optimization_algs/xp_cocolbmorepool10/';
...
for k = 1:nbFun    
    funs{k} = static_optimization_algs.CocoWrapper(funsAll{selectedFuns(k)}, ['f' num2str(selectedFuns(k))], 1, rootPlot);
end
...
parpool(ParpoolNB);
%ugly but parpool is complaining
all_evals1 = cell(nbFun, 1);
all_evals2 = cell(nbFun, 1);

parfor k = 1:length(funs)
    [~, evals] = static_optimization_algs.batchTestOptim(funs(k), algs, settings);
    all_evals1{k} = evals{1, 1};
    all_evals2{k} = evals{1, 2};
end

save(fullfile(rootPlot, 'allPerfsAndEvals'), 'all_evals1', 'all_evals2');
