classdef MoreVsLB
    methods (Static)        
        ...
        function sign = getSignature(optimizerInput)
            sign = ['MoreVsLB' '_' num2str(optimizerInput.epsiKL) '_' num2str(optimizerInput.nbIter) '_' ...
        ...
        function [perf, kls] = optimizeStruct(optimizerInput)
            [perf, kls] = static_optimization_algs.MoreVsLB.optimize(optimizerInput.initDistrib, optimizerInput.epsiKL, optimizerInput.entropyReduction, ...
                optimizerInput.nbSamplesPerIter, optimizerInput.nbInitSamples, optimizerInput.nbIter, optimizerInput.maxIterReuse, ...
                optimizerInput.regularization, optimizerInput.fun, optimizerInput.useImportanceSampling, optimizerInput.videoFile);
        end
        
        %% main function
        function [perf, kls] = optimize(initDistrib, epsiKL, entropyReduc, nbSamplesPerIter, nbInitSamples, nbIter, maxIterReuse, regularization, fun, useImportanceSampling, videoFile)
            perf = zeros(nbIter, 2);
            kls = zeros(nbIter, 1);
