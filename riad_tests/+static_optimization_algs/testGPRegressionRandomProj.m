...
x = bsxfun(@minus, data, mu) * invCC;
y = evals - mean(evals);

featureFunctionObj = static_optimization_algs.RectifiedRandomProjFeatures(2, 50, 1);
featureFunction = @(x) featureFunctionObj.getFeatures(x);
x = featureFunction(x);
...
% hnd = figure;
subplot(1,5,4);
static_optimization_algs.GP.plotGP(gp_rec, x, y, [-1, -2], [3, 6], mu, invCC, featureFunction);
hold on;
plot(optPos(1), optPos(2), '*r', 'MarkerSize', 20);
plot(opt2Pos(1), opt2Pos(2), '*b', 'MarkerSize', 20);
axis([-4,2, -8,4]);
...
for k = 1:length(gp_rec.e)
    ...
    gp.cf{1}.coeffSigma2 = gp_rec.cf{1}.coeffSigma2(k);
    newDist = static_optimization_algs.Normal(dataDis.mu, 1 * dataDis.getCov);
    samplesArgmax(k, :) = static_optimization_algs.GP.localArgmaxSample(gp, x, y, newDist, 800, mu, invCC, featureFunction);
end
...
figure
idx = randperm(length(gp_rec.cf{1}.coeffSigma2));
for k = 1:5
    subplot(1, 5, k);
    gp.lik.sigma2 = gp_rec.lik.sigma2(idx(k));
    gp.cf{1}.coeffSigma2 = gp_rec.cf{1}.coeffSigma2(idx(k));
    static_optimization_algs.GP.plotGP(gp, x, y, [-1, -2], [3, 6], mu, invCC, featureFunction);
end
