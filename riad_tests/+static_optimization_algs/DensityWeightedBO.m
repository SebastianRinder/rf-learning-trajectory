classdef DensityWeightedBO
    methods (Static)
        %%
        function sign = getSignature(optimizerInput)
            sign = ['DensityWeightedBO' '_' num2str(optimizerInput.epsiKL) '_' num2str(optimizerInput.nbIter) '_' ...
        ...
        function [perf, kls] = optimizeStruct(optimizerInput, func)
            ...
            [perf, kls] = static_optimization_algs.DensityWeightedBO.optimize(optimizerInput.initDistrib, optimizerInput.epsiKL, optimizerInput.entropyReduction, ...
                optimizerInput.nbSamplesPerIter, optimizerInput.nbIter, optimizerInput.maxIterReuse, ...
                optimizerInput.minProbaReuse, func, optimizerInput.gpStuffPath, optimizerInput.kernelType, optimizerInput.featureFunction, optimizerInput.yCenteringType, optimizerInput.gpHyperOption, optimizerInput.videoFile);
        end
        
        %% main function
        function [allVals, kls] = optimize(initDistrib, epsiKL, entropyReduc, nbSamplesPerIter, nbIter, maxIterReuse, minProbaReuse, fun, gpStuffPath, kernelType, featureFunction, yCenteringType, gpHyperOption, videoFile)
            kls = zeros(nbIter, 1);
...
%             lik = lik_gaussian(lik,'sigma2_prior', pn);
            ...
            elseif(strcmp(kernelType, 'linear'))
                ...
                error('unrecognized kernel in DensityWeightedBO');
            end
            ...
            for iter = 1:nbIter
                neg_proba_thresh = -.8;
                [newSamples, newVals, gp, gp_rec] = static_optimization_algs.DensityWeightedBO_core_optim.sample(currentDistrib, fun, nbSamplesPerIter, ...
...
%                 [~, quadModel] = static_optimization_algs.Quadratic.learnQuadModel(localSamples, localVals, 1e-6, exp(logInitProbas));
                ...
                if(~isempty(videoFile))
                    frame = static_optimization_algs.DensityWeightedBO.getFrame(fun, currentDistrib, localSamples, x, y, gp, currentDistrib.mu, cholPrec, newSamples, iter, featureFunction);
