classdef AdaptiveMoreStudent
    methods (Static)
        %%
        function sign = getSignature(optimizerInput)
            sign = ['AdaptiveMoreStudent' '_' num2str(optimizerInput.epsiKL) '_' num2str(optimizerInput.nbIter) '_' ...
...
%                 end
        ...
        function [etaStar, isBadModel] = lineSearchStudent(samples, vals, logImportanceProbas, initPolicy, quadFunction, minKL, maxKL, nbSearch, avrRewardLBFun, verbose, desiredEntropy)
            ...
            for k = 1:nbSearch
                ...
                [selection, isTooLess] = static_optimization_algs.ConcentrationInequalities.trimImprobableSampels(logProbas, logImportanceProbas, trimedRatio, minSamples);
                if(isTooLess)
                    rewardLb(k) = -inf;
                else
                    rewardLb(k) = avrRewardLBFun(vals(selection), logImportanceProbas(selection), logProbas(selection));
                end
            ...
            [maxVal, i] = max(rewardLb);
            if(maxVal <= rewardLb(end))
                etaStar = etas(end);
            else
                etaStar = etas(i);
            end
