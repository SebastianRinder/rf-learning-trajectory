...
if isstruct(gp) && numel(gp.jitterSigma2)==1
  ...
  if (isfield(gp.lik.fh,'trcov') && ~isfield(gp,'lik_mono')) ...
    ...
    [chol_Covft, p] = chol(Covft,'lower');
    epsi_chol = 1e-6;
    while p        
        warning('Covft not invertible');
        [chol_Covft, p] = chol(Covft + epsi_chol * eye(size(Covft)),'lower');        
        epsi_chol = epsi_chol * 10;
    end
    sampft = bsxfun(@plus, Eft, chol_Covft*rr);
    if nargout > 1
      sampyt = bsxfun(@plus, Eyt, chol_Covft*rr);
    end
