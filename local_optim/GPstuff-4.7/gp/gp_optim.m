...
if isequal(lower(loss),'e') || (isequal(lower(loss),'loo')) && (isfield(gp.lik.fh,'trcov') || isequal(gp.latent_method, 'EP'))
  switch nargout
    ...
    case 1
      %use cmaes
%       fh_eg=@(ww) gp_e(ww', gp, x, y, 'z', z);
%       opts = cmaes('defaults');
%       opts.Seed = [];
%       opts.TolFun = 1e-3;
%       opts.MaxFunEvals = 500;
%       opts.CMA.active = '2';
%       w = cmaes(fh_eg, w, 2, opts)';
      
      %use fminunc
      opts = optimoptions(@fminunc, 'GradObj', 'on', 'TolFun', 1e-2, 'Algorithm', 'trust-region', 'Display', 'off');
      w = fminunc(fh_eg, w, opts);
